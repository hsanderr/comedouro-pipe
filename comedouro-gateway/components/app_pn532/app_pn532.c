...
// #define PN532_DBG_EN
// #define MIFARE_DBG_EN

// #ifdef PN532_DBG_EN // enable PN532 debugging
// #define ESP_LOGD(TAG, fmt, ...) ESP_LOGD(TAG, fmt, ##__VA_ARGS__)
// #else
// #define ESP_LOGD(TAG, fmt, ...)
// #endif

// #ifdef MIFARE_DBG_EN // enable MIFARE debugging
// #define ESP_LOGD(TAG, fmt, ...) ESP_LOGD(TAG, fmt, ##__VA_ARGS__)
// #else
// #define ESP_LOGD(TAG, fmt, ...)
// #endif

#define PN532_PACK_BUF_SIZE 64 // PN532 packet buffer size

#ifndef BIT_SET_POS
// return variable with only one bit set on position bit_pos (bitN bitN-1 ... bit2 bit1 bit0)
#define BIT_SET_POS(bit_pos) (1 << (bit_pos))
#endif

#define PN532_DELAY(ms) vTaskDelay(ms / portTICK_PERIOD_MS)

#define PN532_ACK_TIMEOUT_MS 1000 ///< Timeout for receiving ACK from PN532

#define PN532_FRAME_DATA_POS 6

//------------------ Defines End ------------------//

//------------------ Variables declarations Start ------------------//

static char *TAG = "Pn532Module";                                         ///< TAG for ESP_LOG*
static uint8_t pn532_ack[] = {0x00, 0x00, 0xFF, 0x00, 0xFF, 0x00};        ///< PN532 ACK frame
static uint8_t pn532_res_fw_ver[] = {0x00, 0xFF, 0x06, 0xFA, 0xD5, 0x03}; ///< PN532 firmware version response
static uint8_t pn532_packet_buf[PN532_PACK_BUF_SIZE];                     ///< PN532 packet buffer

//------------------ Variables declarations End ------------------//

//------------------ Functions prototypes Start ------------------//

static void pn532__read_data(pn532_t *obj, uint8_t *buff, uint8_t n);
static void pn532__write_cmd(pn532_t *obj, uint8_t *cmd, uint8_t cmdlen);
static bool pn532__read_ack(pn532_t *obj);
static bool pn532__is_ready(pn532_t *obj);
static bool pn532__wait_until_ready(pn532_t *obj, uint16_t timeout);
static void pn532__spi_write(pn532_t *obj, uint8_t c);
static uint8_t pn532__spi_read(pn532_t *obj);
...
/**
 * @brief Initialize SPI to communicate with PN532.
 *
 * @param obj Pointer to PN532 object where data and configurations will be stored.
 * @param clk SPI clock GPIO number.
 * @param miso MISO GPIO number.
 * @param mosi MOSI GPIO number.
 * @param cs Chip select GPIO number.
 */
void pn532__spi_init(pn532_t *obj, uint8_t clk, uint8_t miso, uint8_t mosi, uint8_t cs)
{
    // set GPIOs' numbers on PN532 object
    obj->_clk = clk;
    obj->_miso = miso;
    obj->_mosi = mosi;
    obj->_cs = cs;
    ...
    // esp_rom_gpio_pad_select_gpio(obj->_cs);
    // CHANGED_END

    gpio_set_direction(obj->_cs, GPIO_MODE_OUTPUT);   // configure chip select GPIO as output
    gpio_set_level(obj->_cs, 1);                      // leave PN532 chip unselected
    gpio_set_direction(obj->_clk, GPIO_MODE_OUTPUT);  // configure clock GPIO as output
    gpio_set_direction(obj->_mosi, GPIO_MODE_OUTPUT); // configure MOSI GPIO as output
    gpio_set_direction(obj->_miso, GPIO_MODE_INPUT);  // configure MISO GPIO as output
}

/**
 * @brief Wake PN532 up.
 *
 * @param obj Pointer to PN532 object.
 */
void pn532__wakeup(pn532_t *obj)
{
    gpio_set_level(obj->_cs, 0); // select PN532

    PN532_DELAY(1000);

    // send command to wake PN532 up
    pn532_packet_buf[0] = PN532_COMMAND_GETFIRMWAREVERSION;
    pn532__send_cmd_check_ack(obj, pn532_packet_buf, 1, PN532_ACK_TIMEOUT_MS);

    gpio_set_level(obj->_cs, 1); // deselect PN532
}
 ...
 * @param obj Pointer to PN532 object.
 * @return esp_err_t
 * @retval 0 if operation succeeded.
 * @retval 1 if operation failed.
 */
esp_err_t pn532__get_fw_ver(pn532_t *obj, uint8_t *ic_ver, uint8_t *fw_ver, uint8_t *fw_rev)
{
    pn532_packet_buf[0] = PN532_COMMAND_GETFIRMWAREVERSION;

    if (!pn532__send_cmd_check_ack(obj, pn532_packet_buf, 1, PN532_ACK_TIMEOUT_MS))
    {
        return ESP_FAIL;
    }

    // read 12 bytes from response
    pn532__read_data(obj, pn532_packet_buf, 12);

    /**
     * Check response frame
     * start code (0x00, 0xff) + len + lcs (length checksum) + tfi (frame identifier) + first + 0x03
     */
    if (0 != strncmp((char *)pn532_packet_buf, (char *)pn532_res_fw_ver, 6))
    {
        ESP_LOGD(TAG, "Unexpected response!");
        return ESP_FAIL;
    }

    ESP_LOGD(TAG, "IC version: 0x%2.2x (expected 0x32), fw version: 0x%2.2x (expected 0x01), "
                  "fw revision: 0x%2.2x (expected 0x06), support: 0x%2.2x  (expected 0x07)",
             pn532_packet_buf[PN532_FRAME_DATA_POS], pn532_packet_buf[PN532_FRAME_DATA_POS + 1],
             pn532_packet_buf[PN532_FRAME_DATA_POS + 2], pn532_packet_buf[PN532_FRAME_DATA_POS + 3]);

    *ic_ver = pn532_packet_buf[PN532_FRAME_DATA_POS];
    *fw_ver = pn532_packet_buf[PN532_FRAME_DATA_POS + 1];
    *fw_rev = pn532_packet_buf[PN532_FRAME_DATA_POS + 2];
...
/**
 * @brief Send command and wait for ack.
 *
 * @param obj Pointer to PN532 object.
 * @param cmd Command.
...
{
    // write the command
    pn532__write_cmd(obj, cmd, cmdlen);
...
bool pn532_writeGPIO(pn532_t *obj, uint8_t pinstate)
{
    ...
    // Fill command buffer
    pn532_packet_buf[0] = PN532_COMMAND_WRITEGPIO;
    pn532_packet_buf[1] = PN532_GPIO_VALIDATIONBIT | pinstate; // P3 Pins
    pn532_packet_buf[2] = 0x00;                                // P7 GPIO Pins (not used ... taken by SPI)

    ESP_LOGD(TAG, "Writing P3 GPIO: %02x\n", pn532_packet_buf[1]);

    // Send the WRITEGPIO command (0x0E)
    if (!pn532__send_cmd_check_ack(obj, pn532_packet_buf, 3, 1000))
        return 0x0;
    ...
    for (int i = 0; i < 8; i++)
    {
        ESP_LOGD(TAG, " %02x", pn532_packet_buf[i]);
    }
    ESP_LOGD(TAG, "\n");

    int offset = 5;
    return (pn532_packet_buf[offset] == 0x0F);
}
...
{
    pn532_packet_buf[0] = PN532_COMMAND_READGPIO;

    // Send the READGPIO command (0x0C)
    if (!pn532__send_cmd_check_ack(obj, pn532_packet_buf, 1, 1000))
        return 0x0;

    // Read response packet (00 FF PLEN PLENCHECKSUM D5 CMD+1(0x0D) P3 P7 IO1 DATACHECKSUM 00)
    pn532__read_data(obj, pn532_packet_buf, 11);
    ...
    ESP_LOGD(TAG, "P3 GPIO: %02x\n", pn532_packet_buf[p3offset]);
    ESP_LOGD(TAG, "P7 GPIO: %02x\n", pn532_packet_buf[p3offset + 1]);
    ESP_LOGD(TAG, "IO GPIO: %02x\n", pn532_packet_buf[p3offset + 2]);
    // Note: You can use the IO GPIO value to detect the serial bus being used
    switch (pn532_packet_buf[p3offset + 2])
    {
    ...
    return pn532_packet_buf[p3offset];
}
...
bool pn532_SAMConfig(pn532_t *obj)
{
    ...
    int offset = 5;
    return (pn532_packet_buf[offset] == 0x15);
}
...
bool pn532_setPassiveActivationRetries(pn532_t *obj, uint8_t maxRetries)
{
    ...
    pn532_packet_buf[1] = 5;    // Config item 5 (MaxRetries)
    pn532_packet_buf[2] = 0xFF; // MxRtyATR (default = 0xFF)
    pn532_packet_buf[3] = 0x01; // MxRtyPSL (default = 0x01)
    pn532_packet_buf[4] = maxRetries;

    ESP_LOGD(TAG, "Setting MxRtyPassiveActivation to %d\n", maxRetries);

    if (!pn532__send_cmd_check_ack(obj, pn532_packet_buf, 5, 1000))
        return 0x0; // no ACK
...
bool pn532_readPassiveTargetID(pn532_t *obj, uint8_t cardbaudrate, uint8_t *uid, uint8_t *uidLength, uint16_t timeout)
{
    ...
    ESP_LOGD(TAG, "Found %d tags\n", pn532_packet_buf[7]);
    if (pn532_packet_buf[7] != 1)
        return 0;

    uint16_t sens_res = pn532_packet_buf[9];
    sens_res <<= 8;
    sens_res |= pn532_packet_buf[10];
    ESP_LOGD(TAG, "ATQA: %02x\n", sens_res);
    ESP_LOGD(TAG, "SAK: %02x\n", pn532_packet_buf[11]);

    /* Card appears to be Mifare Classic */
    *uidLength = pn532_packet_buf[12];

    for (uint8_t i = 0; i < pn532_packet_buf[12]; i++)
    {
        uid[i] = pn532_packet_buf[13 + i];
    }

    ESP_LOGD(TAG, "UID:");
    for (int i = 0; i < pn532_packet_buf[12]; i++)
    {
        ESP_LOGD(TAG, " %02x", uid[i]);
    }
    ESP_LOGD(TAG, "\n");
...
{
    if (sendLength > PN532_PACK_BUF_SIZE - 2)
    {
    ...
    pn532_packet_buf[0] = 0x40; // PN532_COMMAND_INDATAEXCHANGE;
    pn532_packet_buf[1] = obj->_inListedTag;
    for (i = 0; i < sendLength; ++i)
    {
        pn532_packet_buf[i + 2] = send[i];
    }

    if (!pn532__send_cmd_check_ack(obj, pn532_packet_buf, sendLength + 2, 1000))
    {
        ESP_LOGD(TAG, "Could not send APDU\n");
        return false;
    }

    if (!pn532__wait_until_ready(obj, 1000))
    {
        ESP_LOGD(TAG, "Response never received for APDU...\n");
        return false;
    }

    pn532__read_data(obj, pn532_packet_buf, sizeof(pn532_packet_buf));

    if (pn532_packet_buf[0] == 0 && pn532_packet_buf[1] == 0 && pn532_packet_buf[2] == 0xff)
    {
        uint8_t length = pn532_packet_buf[3];
        if (pn532_packet_buf[4] != (uint8_t)(~length + 1))
        {
            ESP_LOGD(TAG, "Length check invalid %02x%02x\n", length, (~length) + 1);
            return false;
        }
        if (pn532_packet_buf[5] == PN532_PN532TOHOST && pn532_packet_buf[6] == PN532_RESPONSE_INDATAEXCHANGE)
        {
            if ((pn532_packet_buf[7] & 0x3f) != 0)
            {
            ...
            for (i = 0; i < length; ++i)
            {
                response[i] = pn532_packet_buf[8 + i];
            }
        ...
        {
            ESP_LOGD(TAG, "Don't know how to handle this command: %02x\n", pn532_packet_buf[6]);
            return false;
        }
    }
    else
...
{
    pn532_packet_buf[0] = PN532_COMMAND_INLISTPASSIVETARGET;
    pn532_packet_buf[1] = 1;
    pn532_packet_buf[2] = 0;
    ...
    if (!pn532__wait_until_ready(obj, 30000))
    {
        return false;
    }
    ...
    if (pn532_packet_buf[0] == 0 && pn532_packet_buf[1] == 0 && pn532_packet_buf[2] == 0xff)
    {
        ...
        if (pn532_packet_buf[5] == PN532_PN532TOHOST && pn532_packet_buf[6] == PN532_RESPONSE_INLISTPASSIVETARGET)
        {
            if (pn532_packet_buf[7] != 1)
            {
                ESP_LOGD(TAG, "Unhandled number of targets inlisted\n");
                ESP_LOGD(TAG, "Number of tags inlisted: %d\n", pn532_packet_buf[7]);
                return false;
            }

            obj->_inListedTag = pn532_packet_buf[8];
            ESP_LOGD(TAG, "Tag number: %d\n", obj->_inListedTag);
...
uint8_t pn532_mifareclassic_AuthenticateBlock(pn532_t *obj, uint8_t *uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t *keyData)
{
    ...
    {
        ESP_LOGD(TAG, " %02x", obj->_uid[i]);
    }
    ESP_LOGD(TAG, "\n");
    ESP_LOGD(TAG, "Using authentication KEY %c\n", keyNumber ? 'B' : 'A');
    for (int i = 0; i < 6; i++)
    {
        ESP_LOGD(TAG, " %02x", obj->_key[i]);
    }
    ESP_LOGD(TAG, "\n");

    // Prepare the authentication command //
    pn532_packet_buf[0] = PN532_COMMAND_INDATAEXCHANGE; /* Data Exchange Header */
    pn532_packet_buf[1] = 1;                            /* Max card numbers */
    pn532_packet_buf[2] = (keyNumber) ? MIFARE_CMD_AUTH_B : MIFARE_CMD_AUTH_A;
    pn532_packet_buf[3] = blockNumber; /* Block Number (1K = 0..63, 4K = 0..255 */
    memcpy(pn532_packet_buf + 4, obj->_key, 6);
    for (i = 0; i < obj->_uidLen; i++)
    {
        pn532_packet_buf[10 + i] = obj->_uid[i]; /* 4 uint8_t card ID */
    }

    if (!pn532__send_cmd_check_ack(obj, pn532_packet_buf, 10 + obj->_uidLen, 1000))
        return 0;
...
{
    ESP_LOGD(TAG, "Trying to read 16 bytes from block %d\n", blockNumber);

    /* Prepare the command */
    pn532_packet_buf[0] = PN532_COMMAND_INDATAEXCHANGE;
    pn532_packet_buf[1] = 1;               /* Card number */
    pn532_packet_buf[2] = MIFARE_CMD_READ; /* Mifare Read command = 0x30 */
    pn532_packet_buf[3] = blockNumber;     /* Block Number (0..63 for 1K, 0..255 for 4K) */
    ...
    /* Read the response packet */
    pn532__read_data(obj, pn532_packet_buf, 26);
    ...
    memcpy(data, pn532_packet_buf + 8, 16);

    /* Display data for debug if requested */
    ESP_LOGD(TAG, "Block %d\n", blockNumber);
    for (int i = 0; i < 16; i++)
    {
...
{
    ESP_LOGD(TAG, "Trying to write 16 bytes to block %d\n", blockNumber);

    /* Prepare the first command */
    pn532_packet_buf[0] = PN532_COMMAND_INDATAEXCHANGE;
    pn532_packet_buf[1] = 1;                /* Card number */
    pn532_packet_buf[2] = MIFARE_CMD_WRITE; /* Mifare Write command = 0xA0 */
    pn532_packet_buf[3] = blockNumber;      /* Block Number (0..63 for 1K, 0..255 for 4K) */
    memcpy(pn532_packet_buf + 4, data, 16); /* Data Payload */

    /* Send the command */
    if (!pn532__send_cmd_check_ack(obj, pn532_packet_buf, 20, 1000))
    {
...
uint8_t pn532_mifareultralight_ReadPage(pn532_t *obj, uint8_t page, uint8_t *buffer)
{
    ...
    pn532_packet_buf[1] = 1;               /* Card number */
    pn532_packet_buf[2] = MIFARE_CMD_READ; /* Mifare Read command = 0x30 */
    pn532_packet_buf[3] = page;            /* Page Number (0..63 in most cases) */
    ...
    if (pn532_packet_buf[7] == 0x00)
    {
        ...
        /* bytes                                              */
        memcpy(buffer, pn532_packet_buf + 8, 4);
    }
    else
    {
...
uint8_t pn532_mifareultralight_WritePage(pn532_t *obj, uint8_t page, uint8_t *data)
{
    ...
    ESP_LOGD(TAG, "Trying to write 4 uint8_t page %d\n", page);

    /* Prepare the first command */
    pn532_packet_buf[0] = PN532_COMMAND_INDATAEXCHANGE;
    pn532_packet_buf[1] = 1;                           /* Card number */
    pn532_packet_buf[2] = MIFARE_ULTRALIGHT_CMD_WRITE; /* Mifare Ultralight Write command = 0xA2 */
    pn532_packet_buf[3] = page;                        /* Page Number (0..63 for most cases) */
    memcpy(pn532_packet_buf + 4, data, 4);             /* Data Payload */

    /* Send the command */
    if (!pn532__send_cmd_check_ack(obj, pn532_packet_buf, 8, 1000))
    {
        ESP_LOGD(TAG, "Failed to receive ACK for write command\n");
        // Return Failed Signal
        return 0;
...
*/
/**************************************************************************/
bool pn532__read_ack(pn532_t *obj)
{
    uint8_t ackbuff[6];

    pn532__read_data(obj, ackbuff, 6);
...
*/
/**************************************************************************/
bool pn532__is_ready(pn532_t *obj)
{
    gpio_set_level(obj->_cs, 0);
    PN532_DELAY(10);
    pn532__spi_write(obj, PN532_SPI_STATREAD);
    // read uint8_t
    uint8_t x = pn532__spi_read(obj);

    gpio_set_level(obj->_cs, 1);

    // Check if status is ready.
    return x == pn532__spi_readY;
}
...
*/
/**************************************************************************/
bool pn532__wait_until_ready(pn532_t *obj, uint16_t timeout)
{
    uint16_t timer = 0;
    while (!pn532__is_ready(obj))
    {
        if (timeout != 0)
        {
            timer += 10;
...
*/
/**************************************************************************/
void pn532__read_data(pn532_t *obj, uint8_t *buff, uint8_t n)
{
    gpio_set_level(obj->_cs, 0);
    PN532_DELAY(10);
    pn532__spi_write(obj, PN532_SPI_DATAREAD);

    ESP_LOGD(TAG, "Reading:");
    for (uint8_t i = 0; i < n; i++)
    {
        PN532_DELAY(10);
        buff[i] = pn532__spi_read(obj);
    }
    for (int i = 0; i < n; i++)
    {
...
uint8_t pn532_AsTarget(pn532_t *obj)
{
    ...
    if (!pn532__send_cmd_check_ack(obj, target, sizeof(target), 1000))
        return false;
...
{
    uint8_t length;
    pn532_packet_buf[0] = 0x86;
    if (!pn532__send_cmd_check_ack(obj, pn532_packet_buf, 1, 1000))
    {
        ESP_LOGD(TAG, "Error en ack\n");
        return false;
    }

    // read data packet
    pn532__read_data(obj, pn532_packet_buf, 64);
    length = pn532_packet_buf[3] - 3;
...
uint8_t pn532_setDataTarget(pn532_t *obj, uint8_t *cmd, uint8_t cmdlen)
{
    ...
    if (!pn532__send_cmd_check_ack(obj, cmd, cmdlen, 1000))
        return false;
...
*/
/**************************************************************************/
void pn532__write_cmd(pn532_t *obj, uint8_t *cmd, uint8_t cmdlen)
{
    uint8_t checksum;
    ...
    pn532__spi_write(obj, PN532_SPI_DATAWRITE);

    checksum = PN532_PREAMBLE + PN532_PREAMBLE + PN532_STARTCODE2;
    pn532__spi_write(obj, PN532_PREAMBLE);
    pn532__spi_write(obj, PN532_PREAMBLE);
    pn532__spi_write(obj, PN532_STARTCODE2);

    pn532__spi_write(obj, cmdlen);
    pn532__spi_write(obj, ~cmdlen + 1);

    pn532__spi_write(obj, PN532_HOSTTOPN532);
    checksum += PN532_HOSTTOPN532;

    ESP_LOGD(TAG, " %02x %02x %02x %02x %02x %02x", (uint8_t)PN532_PREAMBLE, (uint8_t)PN532_PREAMBLE, (uint8_t)PN532_STARTCODE2, (uint8_t)cmdlen, (uint8_t)(~cmdlen + 1), (uint8_t)PN532_HOSTTOPN532);

    for (uint8_t i = 0; i < cmdlen - 1; i++)
    {
        pn532__spi_write(obj, cmd[i]);
        checksum += cmd[i];
        ESP_LOGD(TAG, " %02x", cmd[i]);
    }

    pn532__spi_write(obj, ~checksum);
...
*/
/**************************************************************************/
void pn532__spi_write(pn532_t *obj, uint8_t c)
{
    int8_t i;
...
*/
/**************************************************************************/
uint8_t pn532__spi_read(pn532_t *obj)
{
    int8_t i, x;
    x = 0;
